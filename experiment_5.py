# seq2seq_luong_attention.py
# Train a Seq2Seq RNN with Luong Attention on a toy translation dataset (Multi30k)
...
class LuongAttention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        attention = torch.sum(energy, dim=2)  # [src_len, batch]
...
def main():
    ...
    enc = EncoderRNN(len(SRC.vocab), args.emb_size, args.hidden_size, args.n_layers, args.dropout)
    attn = LuongAttention(args.hidden_size)
    dec = DecoderRNN(len(TRG.vocab), args.emb_size, args.hidden_size, args.n_layers, args.dropout, attn)
    ...
    for epoch in range(args.epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), 'seq2seq_luong.pt')
